...
export async function GET() {
  const [vendas, despesas, insumos, faturamentos] = await Promise.all([
    ...
    prisma.faturamentoMensal.findMany({ orderBy: [{ ano: 'asc' }, { mes: 'asc' }] }),
  ])

  // Totais gerais
  const faturamentoTotal = vendas.reduce((acc: number, v: {total: number|string}) => acc + Number(v.total), 0)

  // Custo operacional (soma custo insumos de cada venda)
  let custoTotal = 0
  const vendasPorDia: Record<string, { receita: number; custo: number }> = {}
  const porProduto: Record<number, { nome: string; qty: number; receita: number }> = {}
  ...
  for (const venda of vendas) {
    ...
    for (const item of venda.itens) {
      ...
      for (const pi of item.produto.insumos) {
        const cuUnit = Number(pi.insumo.rsPago) / Number(pi.insumo.volumeEmbalagem)
      ...
      if (!porProduto[item.produtoId]) {
        porProduto[item.produtoId] = { nome: item.produto.nome, qty: 0, receita: 0 }
      }
  ...
  const totalFixas = despesas
    .filter((d: {tipo: string; valor: number|string}) => d.tipo === 'fixa')
    .reduce((acc: number, d: {valor: number|string}) => acc + Number(d.valor), 0)

  const totalVariaveis = despesas
    .filter((d: {tipo: string; valor: number|string}) => d.tipo === 'variavel')
    .reduce((acc: number, d: {valor: number|string}) => acc + Number(d.valor), 0) // em %

  // Estoque crítico (zero ou negativo)
  const estoqueCritico = insumos.filter((i: {estoqueAtual: number|string}) => Number(i.estoqueAtual) <= 0)
