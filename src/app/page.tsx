...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const porDia: Record<
      string,
      { dia: string; receita: number; lucro: number }
    > = {};
    ...
    const top5 = Object.values(porProduto)
      ...
      .slice(0, 5);
    const linhas = Object.values(porDia)
  ...
  const cards = [
    ...
    },
    {
      ...
      value: fmt(stats.custoTotal),
      color: 'text-amber-500',
    },
    {
      label: 'Lucro Líquido',
      value: fmt(stats.lucro),
      color: stats.lucro >= 0 ? 'text-emerald-500' : 'text-red-500',
    },
    {
      label: 'Total de Vendas',
      value: state.vendas.length.toString(),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in">
      <div>
        <h1 className="font-display text-3xl text-stone-900 dark:text-stone-100">
          Dashboard
        </h1>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={stats.linhas}>
                <CartesianGrid
                  strokeDasharray="3 3"
                  stroke="#e5e7eb"
                  className="dark:[&>line]:stroke-neutral-700"
                />
      ...
      <div className="card p-5">
        ...
        {state.insumos.filter((i) => i.estoqueAtual <= 0).length === 0 ? (
          <p className="text-stone-400 text-sm">
            Todos os insumos com estoque positivo ✓
          </p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {state.insumos
              .filter((i) => i.estoqueAtual <= 0)
              .map((i) => (
                <span
                  key={i.id}
                  className="badge bg-red-100 dark:bg-red-950 text-red-600 dark:text-red-400"
                >
                  ⚠️ {i.nome}
                </span>
              ))}
